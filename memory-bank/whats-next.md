...
4. ~~**HUD screen**: port v10 card layout (header, portrait slot with tap-to-upload → vault/assets/portrait.png, POWER LEVEL SCAN wired to real PL scores, energy reading, radar showing real hub positions, folder cards with live counts, battle log reading vault files where source != user).~~ ✅ Done
5. ~~**Note CRUD**: create/edit/delete via UI writes .md to vault; folder picker; markdown editor (textarea + preview is fine).~~ ✅ Done
