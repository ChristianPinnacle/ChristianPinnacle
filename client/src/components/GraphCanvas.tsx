...
function drawEdge(
  ...
  if (edge.type === 'ai') {
    const pending = edge.accepted !== true;
    ctx.strokeStyle = pending ? 'rgba(245,197,66,0.55)' : 'rgba(245,197,66,0.85)';
...
export function GraphCanvas({ nodes: rawNodes, edges, onNodeTap, ignitedIds = [] }: GraphCanvasProps) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const edge of edges) {
        ...
        if (a && b) drawEdge(ctx, a, b, edge);
      }

      for (const node of layoutNodes) {
        drawNode(ctx, node, node.id === selectedId, ignitedSet.has(node.id), t);
      }
  ...
  }, [layoutNodes, edges, transform, selectedId, ignitedIds]);
